...
class NumberSchema(BaseSchema):
    ...
    def positive(self) -> Self:
        self.checks['positive'] = lambda value: value is None or value > 0
        return self

    def range(self, min_value: Number, max_value: Number) -> Self:
        self.checks['range'] = (
            lambda value: value is None or min_value <= value <= max_value
        )
        return self
...
class DictSchema(BaseSchema):
    def __init__(self, validator: Validator) -> None:
        ...
        self.shape_validators: dict[Hashable, BaseSchema] = {}

    def shape(self, structure: dict[Hashable, BaseSchema]) -> Self:
        self.shape_validators = structure
        self.checks['shape'] = self._validate_shape
        return self

    def _validate_shape(self, value: dict[Hashable, Any]) -> bool:
        return all(
            schema.is_valid(value.get(key))
